...
func SealPreCommitPhase1(
	...
	resp := generated.FilSealPreCommitPhase1(sp, cacheDirPath, stagedSectorPath, sealedSectorPath, uint64(sectorNum), proverID, to32ByteArray(ticket))
	resp.Deref()
...
func SealCommitPhase1(
	...
	resp := generated.FilSealCommitPhase1(sp, commR, commD, cacheDirPath, sealedSectorPath, uint64(sectorNum), proverID, to32ByteArray(ticket), to32ByteArray(seed))
	resp.Deref()
...
type PoStVanPubin struct {
    ChallengeCount int `json:"challengecount"`
    SectorCount    int `json:"sectorcount"`
}

type WindowPoStVanPubin struct {
    Sectoridx int `json:"sectoridx"`
    PoStVanPubin
}

func GetPartitionSectorCount(ctx context.Context, prooftype int) (int, error) {
    s_vanp, err := GetPartitionVanillaParams(ctx, abi.RegisteredPoStProof(prooftype))
    if err != nil {
        return 2, err
    }
    vanp := PoStVanPubin{}
    err = json.Unmarshal([]byte(s_vanp), &vanp)
    if err != nil {
        return 2, err
    }
    return vanp.SectorCount, nil
}

func GetPartitionVanillaParams(
    ctx context.Context,
    proofType abi.RegisteredPoStProof,
    ) (string, error) {
    pp, err := toFilRegisteredPoStProof(proofType)
    ...
    resp := generated.GetPartitionVanillaParams(
        pp,
    )
