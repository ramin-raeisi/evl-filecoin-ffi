...
func GenerateWindowPartitionPoStWithVanilla(
    proofType abi.RegisteredPoStProof,
    minerID abi.ActorID,
    randomness abi.PoStRandomness,
    proofs [][]byte,
    ...
    secIdxs := make([]uint64, len(sectorIdxs))
    for i, sid := range sectorIdxs {
        secIdxs[i] = uint64(sid)
    }

    resp := generated.FilGenerateWindowPartitionPostWithVanilla(
        pp,
        to32ByteArray(randomness),
        proverID,
        fproofs, uint(len(proofs)),
        secIdxs, uint(len(secIdxs)),
    )
    resp.Deref()
    resp.ProofsPtr = make([]generated.FilPoStProof, resp.ProofsLen)
    resp.Deref()
