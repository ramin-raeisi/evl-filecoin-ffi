...
use filecoin_proofs_api::seal::{
    ...
    generate_piece_commitment, get_seal_inputs, seal_commit_phase2,
...
#[no_mangle]
pub unsafe extern "C" fn fil_generate_labels_bench(
    registered_proof: fil_RegisteredSealProof,
    cache_dir_path: *const libc::c_char,
    sector_id: u64,
    prover_id: fil_32ByteArray,
    ...
    catch_panic_response(|| {
        ...
        info!("generate_labels_bench: start");
        ...
        let response: fil_SealPreCommitPhase1Response = Default::default();

        filecoin_proofs_api::seal::generate_labels_bench(
            registered_proof.into(),
            c_str_to_pbuf(cache_dir_path),
            prover_id.inner,
            SectorId::from(sector_id),
        ...
        info!("generate_labels_bench: finish");
...
#[no_mangle]
pub unsafe extern "C" fn fil_calibrate_seal_commit_phase2(
    seal_commit_phase1_output_ptr: *const u8,
    seal_commit_phase1_output_len: libc::size_t,
    sector_id: u64,
    prover_id: fil_32ByteArray,
    ...
    catch_panic_response(|| {
        ...
        let result = scp1o.and_then(|o| {
            filecoin_proofs_api::seal::calibrate_seal_commit_phase2(
                o,
                prover_id.inner,
                SectorId::from(sector_id),
            )
...
#[no_mangle]
pub unsafe extern "C" fn get_post_vanilla_params(
    registered_proof: fil_RegisteredPoStProof,
) -> *mut fil_GeneratePoStVanillaResponse {
    catch_panic_response(|| {
        init_log();

        info!("get_post_vanilla_params: start");

        let mut response = fil_GeneratePoStVanillaResponse::default();
        let result = filecoin_proofs_api::post::get_post_vanilla_params(
            registered_proof.into(),
        );
        ...
        match result {
            Ok(output) => {
                response.status_code = FCPResponseStatus::FCPNoError;
                response.vanilla = rust_str_to_c_str(output);
            }
            Err(err) => {
                response.status_code = FCPResponseStatus::FCPUnclassifiedError;
                response.error_msg = rust_str_to_c_str(format!("{:?}", err));
        ...
        info!("get_post_vanilla_params: finish");
...
#[no_mangle]
pub unsafe extern "C" fn fil_generate_window_partition_post_with_vanilla(
    registered_proof: fil_RegisteredPoStProof,
    randomness: fil_32ByteArray,
    prover_id: fil_32ByteArray,
    vanilla_proofs_ptr: *const fil_VanillaProof,
    vanilla_proofs_len: libc::size_t,
    sector_idxs_ptr: *const u64,
    sector_idxs_len: libc::size_t,
) -> *mut fil_GenerateWindowPoStResponse {
    catch_panic_response(|| {
        init_log();

        info!("generate_window_partition_post_with_vanilla: start");
        ...
        let slice: &[u64] = std::slice::from_raw_parts(sector_idxs_ptr, sector_idxs_len);
        let sector_idxs: Vec<u64> = slice.to_vec().iter().cloned().map(Into::into).collect();

        let result = filecoin_proofs_api::post::generate_window_partition_post_with_vanilla(
            registered_proof.into(),
            &randomness.inner,
            prover_id.inner,
            &vanilla_proofs,
        ...
        info!("generate_window_partition_post_with_vanilla: finish");
